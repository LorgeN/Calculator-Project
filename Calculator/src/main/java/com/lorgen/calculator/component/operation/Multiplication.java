...
public class Multiplication extends UnorderedListComponent {

    public Multiplication(ExpressionComponent... components) {
        super(components);
    }
    ...
    public Number asNumber() throws CalculationException {
        ...
        for (int i = 1; i < this.getComponents().size(); i++) {
            number.multiply(this.getComponents().get(i).asNumber());
        }
